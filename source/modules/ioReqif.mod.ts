...
//				console.debug( 'addDescProperty', eC, eL );
				...
				/*	function addType( L, ty ) {
						// Add the type or class, if not yet defined:
						L = pr[L];
						if (!Array.isArray(L)) L = [];
						// Avoid duplicates:
						if( indexById( L, ty.id )<0 ) L.unshift( ty );
					} */
				...
				for( var j=eL.length-1; j>-1; j-- ) {
					...
					//	addType( "propertyClasses", app.standardTypes.get("propertyClass","PC-Description") );
					...
					//	addType( "dataTypes", app.standardTypes.get("dataType","DT-FormattedText") );
